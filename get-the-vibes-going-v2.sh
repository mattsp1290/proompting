        ...
        if [ -f "$MIGRATE_SCRIPT" ]; then
            if command -v python3 &> /dev/null; then
                ...
                if python3 -c "import yaml" 2>/dev/null; then
                    ...
                    python3 "$MIGRATE_SCRIPT" "$TASKS_YAML" -o "$MIGRATION_OUTPUT"
                    ...
                    print_info "Then run: python3 $MIGRATE_SCRIPT $TASKS_YAML"
                fi
            else
                print_info "Python 3 not found. Manual migration required."
            fi
        else
